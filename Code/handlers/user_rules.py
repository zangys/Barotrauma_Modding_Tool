...
class UserRulesManager:
    ...
    def add_rule(cls, before_id: str, after_id: str) -> Tuple[bool, str]:
        """
        Adds a rule: 'before_id' loads BEFORE 'after_id'.
        Logic: 'after_id' DEPENDS ON 'before_id'.
        graph edge: before_id -> after_id
        """
        ...
        # Check for Cycle (BFS)
        # We want to add Edge: A -> B (A loads before B)
        # Cycle exists if there is already a path from B -> ... -> A
        if cls._check_path_exists(start_node=after_id, end_node=before_id):
             return False, "Circular dependency detected! This rule would create a loop."

        cls._rules.append({
            "action": "load_before",
            "subject": before_id, # The one that must be first (Parent)
            "target": after_id    # The one that must be second (Child)
        })
        cls.save()
        return True, "Success"
